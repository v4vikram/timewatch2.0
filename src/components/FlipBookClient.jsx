...
export default function FlipBookClient() {
  ...
  // Preload images
  useEffect(() => {
    const firstBatch = imagePaths.slice(0, 15);
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      ...
      if (e.deltaY > 0 && currentPage < totalPages - 1) handleNext();
      else if (e.deltaY < 0 && currentPage > 0) handlePrev();
    };
    ...
    <div className="flex flex-col justify-center items-center bg-[#e0e0e0] h-screen overflow-hidden relative">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-4">
        <a
          ...
          download
          className="p-3 rounded-full border border-gray-600 bg-white text-gray-800 hover:bg-gray-100 flex items-center"
        >
        ...
        <button
          className={`p-3 rounded-full border border-gray-600 bg-white ${
            audioEnabled ? "bg-green-500 text-white" : "text-gray-800"
          }`}
